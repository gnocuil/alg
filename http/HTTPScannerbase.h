...
#ifndef HTTPScannerBASE_H_INCLUDED
#define HTTPScannerBASE_H_INCLUDED
...
public:
    HTTPScannerBase(HTTPScannerBase const &other)             = delete;
    HTTPScannerBase &operator=(HTTPScannerBase const &rhs)    = delete;
...
protected:
    HTTPScannerBase(std::istream &in, std::ostream &out);
    HTTPScannerBase(std::string const &infilename, std::string const &outfilename);
...
inline std::ostream &HTTPScannerBase::out()
{
    return *d_out;
}

inline void HTTPScannerBase::push(size_t ch)
{
    d_input.reRead(ch);
}

inline void HTTPScannerBase::push(std::string const &str)
{
    d_input.reRead(str, 0);
}

inline bool HTTPScannerBase::atFinalState()
{
    return d_final.notAtBOL.rule != std::numeric_limits<size_t>::max() || 
            (d_atBOL && d_final.atBOL.rule != std::numeric_limits<size_t>::max());
}

inline void HTTPScannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void HTTPScannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &HTTPScannerBase::matched() const
{
    return d_matched;
}

inline HTTPScannerBase::StartCondition__ HTTPScannerBase::startCondition() const
{
    return d_startCondition;
}

inline std::string const &HTTPScannerBase::filename() const
{
    return d_filename;
}

inline void HTTPScannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t HTTPScannerBase::length() const
{
    return d_matched.size();
}

inline void HTTPScannerBase::leave(int retValue) const
{
    throw static_cast<Leave__>(retValue);
}

inline size_t HTTPScannerBase::lineNr() const
{
    return d_input.lineNr();
}

inline void HTTPScannerBase::more()
{
    d_more = true;
}

inline void HTTPScannerBase::begin(StartCondition__ startCondition)
{
    d_dfaBase__ = 
        s_dfaBase__[static_cast<int>(d_startCondition = startCondition)];
}

inline size_t HTTPScannerBase::state__() const
{
    return d_state;
}

inline size_t HTTPScannerBase::get__()
{
    return d_input.get();
}

inline bool HTTPScannerBase::return__()
{
    return d_return;
}

inline void HTTPScannerBase::noReturn__()
{
    d_return = false;
}


#endif //  HTTPScannerBASE_H_INCLUDED

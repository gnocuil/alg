...
#ifndef FTPScannerBASE_H_INCLUDED
#define FTPScannerBASE_H_INCLUDED
...
public:
    FTPScannerBase(FTPScannerBase const &other)             = delete;
    FTPScannerBase &operator=(FTPScannerBase const &rhs)    = delete;
...
protected:
    FTPScannerBase(std::istream &in, std::ostream &out);
    FTPScannerBase(std::string const &infilename, std::string const &outfilename);
...
inline std::ostream &FTPScannerBase::out()
{
    return *d_out;
}

inline void FTPScannerBase::push(size_t ch)
{
    d_input.reRead(ch);
}

inline void FTPScannerBase::push(std::string const &str)
{
    d_input.reRead(str, 0);
}

inline bool FTPScannerBase::atFinalState()
{
    return d_final.notAtBOL.rule != std::numeric_limits<size_t>::max() || 
            (d_atBOL && d_final.atBOL.rule != std::numeric_limits<size_t>::max());
}

inline void FTPScannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void FTPScannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &FTPScannerBase::matched() const
{
    return d_matched;
}

inline FTPScannerBase::StartCondition__ FTPScannerBase::startCondition() const
{
    return d_startCondition;
}

inline std::string const &FTPScannerBase::filename() const
{
    return d_filename;
}

inline void FTPScannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t FTPScannerBase::length() const
{
    return d_matched.size();
}

inline void FTPScannerBase::leave(int retValue) const
{
    throw static_cast<Leave__>(retValue);
}

inline size_t FTPScannerBase::lineNr() const
{
    return d_input.lineNr();
}

inline void FTPScannerBase::more()
{
    d_more = true;
}

inline void FTPScannerBase::begin(StartCondition__ startCondition)
{
    d_dfaBase__ = 
        s_dfaBase__[static_cast<int>(d_startCondition = startCondition)];
}

inline size_t FTPScannerBase::state__() const
{
    return d_state;
}

inline size_t FTPScannerBase::get__()
{
    return d_input.get();
}

inline bool FTPScannerBase::return__()
{
    return d_return;
}

inline void FTPScannerBase::noReturn__()
{
    d_return = false;
}


#endif //  FTPScannerBASE_H_INCLUDED
